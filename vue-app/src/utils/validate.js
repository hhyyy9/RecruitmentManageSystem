...
export const validatePsdReg = (rule, value, callback) => {
  ...
  if (!/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,18}$/.test(value)) {
    callback(new Error('请输入6-18位英文字母、数字，至少包含两种'))
  } else {
    callback()
  }
}
...
export const validateCode = (rule, value, callback) => {
  ...
  if (!/^(?![0-9]*$)(?![a-zA-Z]*$)[a-zA-Z0-9]$/.test(value)) {
    callback(new Error('账号必须为字母和数字组合'))
  } else {
    callback()
  }
}
...
// 30、特殊字符及表情验证
export const symbolValidator = (rule, value, callback) => {
  let reg = /[`~!@#$%^&*()+=|{}':;',/\/\[\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]|[\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/;
  if (reg.test(value)) {
    callback(new Error('输入内容包含不支持的特殊字符或表情，请删除重新输入'))
  } else {
    callback();
  }
}
...
// 31、特殊表情验证
export const emojiValidator = (rule, value, callback) => {
  let reg = /[\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/;
  if (reg.test(value)) {
    callback(new Error('输入内容包含表情，请删除重新输入'))
  } else {
    callback();
  }
}
...
// 32、用户名校验
export const validateUnReg = (rule, value, callback) => {
  if (!value) {
    return callback(new Error('请输入用户名'))
  }
  if (!/^(?![\d]+$)(?![a-zA-Z]+$)(?![^\da-zA-Z]+$)([^_]){6,20}$/.test(value)) {
    callback(new Error('请输入6-18位英文字母、数字或者下划线，且字母、数字和下划线至少包含两种'))
  } else {
    callback()
  }
}
//33、中文、数字、字母、符号
export const validatePrizeName = (rule, value, callback) => {
  if (!value) {
    return callback(new Error('请输入奖品名称'))
  }
  let reg = /^[a-zA-Z0-9`~!@#$%^&*()+=|{}':;',/\/\[\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？\u4e00-\u9fa5]{3,16}$/
  if (!reg.test(value)) {
    callback(new Error('请输入3-16位中文、字母、数字、符号'))
  } else {
    callback()
  }
}
...
//35、只能输入中文、字母
export const validateIsChineseAndEnglish = (rule, value, callback) => {
  if (!value) {
...
//36、字母+数字
export function validateIsNumberAndEnglish(rule, value, callback) {
  //是否是一定包含字母和数字
...
// 37、验证是否整数
export function isOrganType(rule, value, callback) {
  if (!value) {
